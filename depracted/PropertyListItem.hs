...
class PropertyListAlgebra pl => PropertyListItem pl i where
    ...
    listToPropertyList :: List pl i -> pl
    listToPropertyList      = plArray . map toPropertyList
    
    listFromPropertyList :: pl -> Maybe (List pl i)
    listFromPropertyList (viewPListAlgebra -> Right (PLArray x))    = mapM fromPropertyList x
    listFromPropertyList _ = Nothing

instance PropertyListItem pl a => PropertyListItem pl [a] where
...
tryAlterDictionaryEntryM k f (Just (viewPListAlgebra -> Right (PLDict d))) = do
...
tryAlterDictionaryEntry k f (Just (viewPListAlgebra -> Right (PLDict d))) = (fmap plDict (alterDictionaryEntry k f (Just d)), True)
...
instance PropertyListItem ByteString where
    ...
    fromPropertyList (viewPListAlgebra -> Right (PLData x))   = Just x
    fromPropertyList (viewPListAlgebra -> Right (PLString x)) = Just (toStrictByteString x)
    fromPropertyList _ = Nothing

instance PropertyListItem Lazy.ByteString where
    toPropertyList = plData . toStrictByteString
    fromPropertyList (viewPListAlgebra -> Right (PLData   x)) = Just (toLazyByteString x)
    fromPropertyList (viewPListAlgebra -> Right (PLString x)) = Just (toLazyByteString x)
    fromPropertyList _ = Nothing

instance PropertyListItem UTCTime where
    toPropertyList = plDate
    fromPropertyList (viewPListAlgebra -> Right (PLDate x)) = Just x
    fromPropertyList _ = Nothing
...
instance PropertyListItem a => PropertyListItem (M.Map String a) where
    ...
    fromPropertyList (viewPListAlgebra -> Right (PLDict x)) = fmapM fromPropertyList x
...
instance PropertyListItem Double where
    ...
    fromPropertyList (viewPListAlgebra -> Right (PLInt i)) =  Just (fromInteger i)
    fromPropertyList (viewPListAlgebra -> Right (PLReal d)) = Just d
    fromPropertyList (viewPListAlgebra -> Right (PLString s)) = case reads s of
        [(d, "")] -> Just d
        _ -> Nothing
    fromPropertyList _ = Nothing
...
instance PropertyListItem Integer where
    ...
    fromPropertyList (viewPListAlgebra -> Right (PLInt i)) = Just i
    fromPropertyList (viewPListAlgebra -> Right (PLReal d)) = case properFraction d of
        (i, 0) -> Just i
        _ -> Nothing
    fromPropertyList (viewPListAlgebra -> Right (PLString s)) = case reads s of
        [(i, "")] -> Just i
...
instance PropertyListItem Char where
    ...
    fromPropertyList (viewPListAlgebra -> Right (PLString [c])) = Just c
    fromPropertyList _ = Nothing
    
    listToPropertyList = plString
    listFromPropertyList (viewPListAlgebra -> Right (PLString x)) = Just x
    listFromPropertyList (viewPListAlgebra -> Right (PLData x))   = Just (fromStrictByteString x)
    listFromPropertyList (viewPListAlgebra -> Right (PLBool True)) = Just "YES"
    listFromPropertyList (viewPListAlgebra -> Right (PLBool False)) = Just "NO"
    listFromPropertyList (viewPListAlgebra -> Right (PLInt i)) = Just (show i)
    listFromPropertyList (viewPListAlgebra -> Right (PLReal d)) = Just (show d)
    listFromPropertyList other = Nothing

instance PropertyListItem Bool where
    toPropertyList = plBool
    fromPropertyList (viewPListAlgebra -> Right (PLBool d)) = Just d
    fromPropertyList (viewPListAlgebra -> Right (PLString b))
...
instance PropertyListItem UnparsedPlistItem where
    toPropertyList = return
    fromPropertyList (viewPListAlgebra -> Left d) = Just d
    fromPropertyList _ = Nothing
